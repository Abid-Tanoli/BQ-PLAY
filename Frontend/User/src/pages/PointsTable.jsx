...
export default function PointsTable() {
  ...
  useEffect(() => {
    ...
    // Fetch points table
    // Assuming there is an endpoint /matches/points-table or we calculate it
    // For now, let's try to fetch if it exists, otherwise we'll mock or calculate
    fetch(`${import.meta.env.VITE_API_URL}/matches/points-table`)
      .then(res => res.json())
      .then(data => {
            ...
            <table className="w-full text-left min-w-[800px]">
              ...
              <tbody className="divide-y divide-slate-50 italic">
                {table.length > 0 ? table.map((t, i) => (
                  <tr key={t.teamId || t.team} className="group hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-8 py-6">
                       <div className="flex items-center gap-4">
                          <div className="w-10 h-10 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center p-2">
                             {t.logo ? <img src={t.logo} className="w-full h-full object-contain" /> : <div className="w-full h-full bg-[#031d44] rounded-lg flex items-center justify-center font-black text-white text-xs">{t.team?.charAt(0)}</div>}
                          </div>
                          <span className="font-black text-slate-800 uppercase tracking-tighter italic">{t.team}</span>
                       </div>
                    </td>
                    <td className="px-4 py-6 font-bold text-slate-600 text-center">{t.played}</td>
                    <td className="px-4 py-6 font-black text-green-600 text-center">{t.won}</td>
                    <td className="px-4 py-6 font-black text-red-600 text-center">{t.lost}</td>
                    <td className="px-4 py-6 font-bold text-slate-400 text-center italic">{t.nrr || "0.000"}</td>
                    <td className="px-8 py-6 font-black text-blue-600 text-2xl text-center bg-blue-50/50">{t.points}</td>
                  </tr>
                )) : (
                  <tr>
