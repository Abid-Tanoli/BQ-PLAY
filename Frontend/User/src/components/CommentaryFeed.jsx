...
const CommentaryFeed = ({ match, live = true }) => {
  ...
  return (
    <div className="bg-cric-card dark:bg-slate-800 rounded-lg shadow-lg overflow-hidden">
      {/* Header */}
      <div className="bg-gradient-to-r from-slate-800 to-slate-900 dark:from-slate-900 dark:to-black px-6 py-4">
        ...
        {live && (
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-xs text-cric-muted">Live Updates</span>
          </div>
        )}
      </div>
      ...
      <div ref={scrollRef} className="max-h-[600px] overflow-y-auto">
        {reversedOvers.map((over, overIdx) => (
          <div key={over.overNumber} className="border-b dark:border-slate-700 last:border-b-0">
            {/* Over Header */}
            <div className="bg-cric-bg dark:bg-slate-700 px-4 py-3 sticky top-0 z-10">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <span className="text-lg font-black text-cric-text dark:text-white">
                ...
                <div className="text-xs font-bold text-cric-muted dark:text-slate-300">
            ...
            <div className="divide-y dark:divide-slate-700">
              {[...over.balls].reverse().map((ball, ballIdx) => {
                ...
                return (
                  <div
                    key={ballIdx}
                    className="px-4 py-3 hover:bg-cric-bg dark:hover:bg-slate-750 transition-colors"
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-sm text-cric-text dark:text-white mb-1">
                          {summaryLine}
                        </p>
                        ...
                        {(ball.vividCommentary || ball.commentary) && (
                          <p className="text-sm text-cric-muted dark:text-slate-300 mt-1 leading-relaxed">
                            {ball.vividCommentary || ball.commentary}
                            {ball.isAi && (
                              <span className="ml-2 inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-black bg-gradient-to-r from-purple-500/20 to-blue-500/20 text-purple-600 dark:text-purple-400 uppercase tracking-wider border border-purple-500/30">
                                AI
                              </span>
                            )}
                          </p>
                        ...
                        {dataPoints.length > 0 && (
                          <div className="mt-2 flex flex-wrap gap-1.5">
                            {dataPoints.map(dp => (
                              <span key={dp.label} className={`px-2 py-0.5 rounded text-[9px] font-bold border ${tagColors[dp.type] || "bg-cric-bg dark:bg-slate-700 text-cric-muted dark:text-slate-300"} uppercase tracking-wide`}>
                                {dp.label}
                              </span>
                            ))}
                          </div>
