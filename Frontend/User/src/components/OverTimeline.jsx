...
export default function OverTimeline({ innings }) {
  ...
  const getBallStyle = (ball) => {
    ...
    if (ball.runs === 0) return "bg-cric-bg text-cric-muted border-cric-border";
    return "bg-cric-bg text-cric-text border-cric-border";
  };
  ...
  return (
    <div className="bg-white/50 backdrop-blur-sm p-4 rounded-[2rem] border border-cric-border overflow-hidden">
      <div className="flex items-center gap-4 overflow-x-auto no-scrollbar py-2">
        {allOvers.map((over, oIdx) => (
          <div key={oIdx} className="flex items-center gap-3 shrink-0">
             {/* Over Summary Badge */}
              <div className="flex flex-col items-center justify-center h-12 px-4 bg-cric-bg rounded-2xl border border-cric-border shadow-sm">
                 <span className="text-[10px] font-black text-cric-muted uppercase tracking-tighter">
                   {over.overNumber + 1}{["st", "nd", "rd"][(over.overNumber + 1) % 10 - 1] || "th"}
                </span>
                 <span className="text-xs font-black text-cric-text uppercase">
        ...
        <button className="h-12 px-6 bg-cric-accent text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-blue-900/20 flex items-center gap-2 group whitespace-nowrap ml-4">
           See all
