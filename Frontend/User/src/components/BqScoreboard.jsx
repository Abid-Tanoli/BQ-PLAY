...
const BqScoreboard = ({ match }) => {
  ...
  const renderScorecard = () => (
    <div className="space-y-4">
      <div className="bg-cric-card rounded-xl shadow-lg overflow-hidden">
        <div className="bg-gradient-to-r from-cric-accent to-orange-700 text-white p-4">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-white/80">{match.matchType}</span>
              {match.matchCategory && (
                <span className="text-xs bg-white/20 px-2 py-0.5 rounded-full">{match.matchCategory}</span>
              )}
            </div>
            <span className="text-xs text-white/70">{match.venue}</span>
          </div>
          ...
          <div className="px-4 pt-3 flex gap-3">
            {match.innings.map((inn, idx) => (
              <div key={idx} className={`flex-1 text-center py-2 rounded-lg ${idx === match.currentInnings
                ...
                <p className="text-xs text-cric-muted dark:text-cric-muted font-semibold">{getTeamName(inn.team)}</p>
                <p className={`text-xl font-black ${idx === match.currentInnings ? 'text-cric-text dark:text-white' : 'text-cric-muted dark:text-cric-muted'}`}>
                  {inn.runs}/{inn.wickets}
                </p>
                <p className="text-xs text-cric-muted">({formatOvers(inn.balls)} ov)</p>
              </div>
            ))}
          </div>
        )}

        <div className="p-4 bg-gradient-to-b from-cric-bg to-cric-card">
          <div className="text-center mb-4">
            <div className="text-5xl font-black text-cric-text dark:text-white mb-1">
              {currentInnings.runs}/{currentInnings.wickets}
            </div>
            <div className="text-sm text-cric-muted dark:text-cric-muted">
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="bg-cric-card p-3 rounded-lg text-center shadow-sm">
              <p className="text-xs text-cric-muted dark:text-cric-muted">Run Rate</p>
              <p className="text-xl font-bold text-cric-accent">{runRate}</p>
            </div>
            {requiredRunRate && (
              <div className="bg-cric-card p-3 rounded-lg text-center shadow-sm">
                <p className="text-xs text-cric-muted dark:text-cric-muted">Required RR</p>
          ...
          {target && (
            <div className="bg-cric-bg border border-cric-border p-3 rounded-lg text-center mb-4">
              <p className="text-sm font-semibold text-cric-text">
          ...
          <div className="bg-cric-card rounded-lg p-4 mb-3">
            <h4 className="text-xs font-bold text-cric-muted dark:text-cric-muted uppercase mb-2 flex items-center gap-2">
                ...
                return (
                  <div key={idx} className="flex justify-between items-center">
                    ...
                    <div className="text-sm font-bold dark:text-white flex items-center gap-3">
                      <span>{batStats?.runs || 0} <span className="text-cric-muted font-normal">({batStats?.balls || 0})</span></span>
                      {batStats?.fours > 0 && <span className="text-cric-accent text-xs">4s:{batStats.fours}</span>}
                      {batStats?.sixes > 0 && <span className="text-purple-600 text-xs">6s:{batStats.sixes}</span>}
                      <span className="text-cric-muted text-xs">SR:{sr}</span>
                    </div>
                  </div>
                );
              })}
          ...
          {currentInnings.currentBowler && (
            <div className="bg-cric-card rounded-lg p-4">
              <h4 className="text-xs font-bold text-cric-muted dark:text-cric-muted uppercase mb-2 flex items-center gap-2">
                ...
                {(() => {
                  ...
                  return (
                    <div className="text-sm dark:text-white">
                      ...
                      <span className="text-cric-muted dark:text-cric-muted ml-2">
        ...
        {allRecentBalls.length > 0 && (
          <div className="border-t border-cric-border p-4">
            <h4 className="text-xs font-bold text-cric-muted dark:text-cric-muted uppercase mb-3">Recent Balls</h4>
            <div className="flex gap-2">
              {allRecentBalls.map((ball, idx) => {
                let bgColor = 'bg-slate-300 dark:bg-slate-600 text-slate-700 dark:text-cric-muted';
                let label = '\u2022';
        ...
        {currentPartnership && currentPartnership.runs > 0 && (
          <div className="border-t border-cric-border dark:border-slate-700 p-4 bg-cric-bg">
            <div className="flex justify-between text-sm">
              <span className="text-cric-muted dark:text-cric-muted font-semibold">Partnership</span>
      ...
      {match.innings.length > 0 && (
        <div className="bg-cric-card rounded-xl shadow-lg overflow-hidden">
          <div className="bg-gradient-to-r from-cric-accent to-orange-700 text-white px-5 py-3">
            <h3 className="text-sm font-bold uppercase tracking-wider">Match Stats</h3>
          </div>
          <div className="p-4">
            <div className="flex gap-3 mb-4">
              {summary.inningsSummaries.map((inn, idx) => (
                <div key={idx} className="flex-1 text-center py-3 rounded-lg bg-cric-bg">
                  <p className="text-[10px] text-cric-muted dark:text-cric-muted font-bold uppercase truncate">{inn.team}</p>
                  <p className="text-2xl font-black text-cric-text dark:text-white">{inn.score}</p>
            ...
            <div className="grid grid-cols-2 gap-3">
              {summary.highestScore && (
                <div className="bg-cric-bg rounded-lg p-3 border border-cric-border">
                  <p className="text-[10px] text-cric-accent font-bold uppercase">Highest Score</p>
                  <p className="text-base font-black text-cric-text dark:text-white mt-0.5">
                    {summary.highestScore.name}
                  </p>
                  <p className="text-sm font-bold text-cric-text dark:text-white">
                    {summary.highestScore.runs} <span className="text-cric-muted font-normal text-xs">({summary.highestScore.balls}b, {summary.highestScore.fours}x4, {summary.highestScore.sixes}x6)</span>
                  </p>
                </div>
              )}
              {summary.bestBowling && (
                <div className="bg-cric-bg rounded-lg p-3 border border-cric-border">
                  <p className="text-[10px] text-cric-accent font-bold uppercase">Best Bowling</p>
                  <p className="text-base font-black text-cric-text dark:text-white mt-0.5">
                    {summary.bestBowling.name}
                  </p>
                  <p className="text-sm font-bold text-cric-text dark:text-white">
                    {summary.bestBowling.wickets}/{summary.bestBowling.runs} <span className="text-cric-muted font-normal text-xs">({summary.bestBowling.overs} ov)</span>
                  </p>
                </div>
              )}
            </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-1 bg-cric-card rounded-xl shadow-sm border border-cric-border p-1 overflow-x-auto">
        {TABS.map(tab => (
          <button
            key={tab.id}
            ...
            className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-wider transition-all whitespace-nowrap ${
              activeTab === tab.id
                ? 'bg-cric-accent text-white shadow'
