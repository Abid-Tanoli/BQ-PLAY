          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
            ...
            {selectedType && selectedType !== "single-match" && (
              <div>
                ...
                <input type="number" {...register("totalMatches", { min: 1 })} min="1" placeholder="e.g., 14" className="w-full md:w-1/3 p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-800" />
                ...
                <div className="p-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {["series", "tri-series", "tournament", "world-cup", "champions-trophy", "league"].includes(ev.eventType) && ev.teams?.map(team => {
                      const teamSquad = ev.eventSquads?.find(s => (s.team?._id || s.team) === (team._id || team));
                      const isReady = teamSquad?.players?.length >= 11;
                      return (
                        <button key={team._id || team} onClick={() => openSquadForm(ev, team._id || team)}
                          ...
                          {team.shortName || team.name?.substring(0, 10)} Squad {isReady ? `(${teamSquad.players.length})` : " (Pending)"}
                        </button>
                      );
                    })}
